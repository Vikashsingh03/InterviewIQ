...
// Coaching for CODING questions of a finished interview: how to approach the
// problem, its complexity, a reference solution in the language the candidate
// picks (actually run against the question's test cases), what was missing in
// their code, and tips.
//
// Called from getQuestionCoaching (interview.controller.js) for questions of
// type "coding". Kept in its own file so the controller only needs two tiny
// edits.
...
// the AI answers in plain marked sections instead of JSON, because code
// full of quotes and newlines is exactly what breaks JSON escaping
...
// the code runner being offline must never read as "your solution is wrong"
const isRunnerFailure = (result) =>
  !result.passed &&
  Boolean(result.error) &&
  !/time limit exceeded/i.test(result.error) &&
  /piston|econn|enotfound|network|timeout of|request failed|execution failed/i.test(
    result.error,
  );

const verifySolution = async ({ language, code, testCases }) => {
  try {
    const run = await runTestCases({ language, code, testCases });
    const results = run?.results || [];
    if (!run?.supported || results.length === 0) return { status: "untested" };
    if (results.every(isRunnerFailure)) return { status: "untested" };
...
const buildCodingCoachingMessages = ({
  ...
  const submitted =
    ...
    question.answer.trim()
      ? question.answer.trim().slice(0, 4000)
      : "";
  const examples = dsa.testCases
    .slice(0, 3)
...
// write a solution, run it against the real test cases, and if it fails
// give the AI one chance to fix it using the actual failure
const generateVerifiedSolution = async ({
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (attempt === 0) {
        messages = [
          ...
          { role: "assistant", content: reply },
          {
            role: "user",
            content:
              "That had no usable code. Reply again with ONLY a ===CODE=== section containing the complete program.",
          },
        ];
        continue;
      }
    ...
    const solution = {
      ...
      code,
      verified:
        check.status === "passed" ? true : check.status === "failed" ? false : null,
      ...(check.total ? { passed: check.passed, total: check.total } : {}),
    };
    best = solution;

    if (check.status !== "failed") break; // passed, or can't be tested: no point retrying

    const failureText = check.failures
      .map(
        (f) =>
  ...
  return { solution: best, textSections };
};

// POST /api/interview/coaching/:questionId  (coding questions)
// body: { language? } one of javascript | python | cpp | java
export const handleCodingCoaching = async ({
  ...
  const language = CODING_LANGUAGES.includes(requested)
    ? requested
    : CODING_LANGUAGES.includes(question.language)
      ? question.language
      : "javascript";
  ...
  // this language was already generated: no AI call, no code run
  if (existing?.solutions?.some((s) => s.language === language)) {
    return res.status(200).json({ coaching: toPlain(existing), cached: true });
  }

  const { solution, textSections } = await generateVerifiedSolution({
    dsa,
    language,
    interview,
    question,
  ...
  // first request for this question: also keep the explanation
  if (!existing) {
    ...
    return res.status(200).json({ coaching: toPlain(coaching), cached: false });
  }

  // another language for a question that already has coaching text
  await interviewModel.updateOne(
    { _id: interview._id, "questions._id": questionId },
    { $push: { "questions.$.coaching.solutions": solution } },
  );
