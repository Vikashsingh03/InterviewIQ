...
import { BsRobot, BsMic, BsBarChart, BsBuilding, BsCheckCircleFill, BsPeopleFill, BsPersonFill, BsBullseye } from "react-icons/bs";
...
function Step1Setup({
  ...
  return <div className="min-h-screen relative bg-[#F7F6F3] dark:bg-[#0A0B0D] flex items-center justify-center p-4 sm:p-6 py-12 transition-colors duration-300">
      <style>{`
        ...
        <div className="p-7 sm:p-10 bg-[#F7F6F3] dark:bg-[#0F1115]">
          ...
          <div className="space-y-5">
            ...
            <div className="studio-input relative rounded-2xl border border-[#E5E4E0] dark:border-[#1E2229] bg-white dark:bg-[#0C0E11] transition-all duration-200">
              ...
              <input type="text" placeholder="Enter role — e.g. Frontend Developer" className="w-full pl-11 pr-4 py-3.5 bg-transparent text-[#1C1F24] dark:text-[#EDEEF0] placeholder-[#9AA1AC] dark:placeholder-[#565D68] outline-none text-sm" onChange={e => setRole(e.target.value)} value={role} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 p-1 rounded-2xl bg-[#EFEEEA] dark:bg-[#0C0E11] border border-[#E5E4E0] dark:border-[#1E2229]">
              {MODES.map(m => <button key={m.value} type="button" onClick={() => setMode(m.value)} className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${mode === m.value ? "bg-[#1C1F24] dark:bg-[#EDEEF0] text-white dark:text-[#0A0B0D] shadow-md" : "text-[#5C6472] dark:text-[#8B92A0] hover:text-[#1C1F24] dark:hover:text-[#EDEEF0]"}`}>
                  {m.icon}
                  {m.label}
                </button>)}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                return <motion.button key={t.value} type="button" whileTap={{
                  scale: 0.98
                }} onClick={() => setInterviewType(t.value)} className={`relative flex flex-col items-start gap-1.5 p-3.5 rounded-2xl border text-left transition-all duration-200 ${isSelected ? "border-[#E8A94C] bg-[#E8A94C]/8 shadow-[0_8px_24px_-12px_rgba(232,169,76,0.5)]" : "border-[#E5E4E0] dark:border-[#1E2229] bg-white dark:bg-[#0C0E11] hover:border-[#E8A94C]/30"}`}>
                      ...
                      <span className={`text-sm font-semibold ${isSelected ? "text-[#B27E2E] dark:text-[#E8A94C]" : "text-[#1C1F24] dark:text-[#EDEEF0]"}`}>
                        {t.label}
                      </span>
              ...
              <AnimatePresence>
                {interviewType === "panel" && <motion.p initial={{
                opacity: 0,
                height: 0
              }} animate={{
                opacity: 1,
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 p-1 rounded-2xl bg-[#EFEEEA] dark:bg-[#0C0E11] border border-[#E5E4E0] dark:border-[#1E2229]">
                {LANGUAGES.map(l => <button key={l.value} type="button" onClick={() => setLanguage(l.value)} className={`flex flex-col items-center justify-center gap-0.5 py-2.5 rounded-xl transition-all duration-200 ${language === l.value ? "bg-[#1C1F24] dark:bg-[#EDEEF0] text-white dark:text-[#0A0B0D] shadow-md" : "text-[#5C6472] dark:text-[#8B92A0] hover:text-[#1C1F24] dark:hover:text-[#EDEEF0]"}`}>
                    <span className="text-sm font-semibold">{l.label}</span>
                    <span className="text-[10px] opacity-70">{l.sub}</span>
                  </button>)}
              </div>
              {language !== "english" && <p className="mt-1.5 text-[11px] text-[#9AA1AC] dark:text-[#565D68] pl-1">
            ...
            <div>
              ...
              <AnimatePresence>
                {showJobDescription && <motion.div initial={{
                opacity: 0,
                height: 0
              }} animate={{
                opacity: 1,
                ...
                height: 0
                  ...
                  scale: 0.98
                    ...
                    {matchError && <div className="mt-3 bg-[#E8A94C]/8 border border-[#E8A94C]/25 rounded-xl p-3 flex items-start gap-2">
                    ...
                    <AnimatePresence>
                      {matchResult && <motion.div initial={{
                    opacity: 0,
                    y: 10
                  }} animate={{
                    opacity: 1,
                    ...
                    y: -8
                        ...
                        color: matchResult.matchScore >= 75 ? "#22c55e" : matchResult.matchScore >= 40 ? "#B27E2E" : "#ef4444"
                          ...
                          {matchResult.matchedSkills.length > 0 && <div>
                              <p className="text-xs font-medium text-[#5C6472] dark:text-[#9AA1AC] mb-1.5">
                                You match on
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {matchResult.matchedSkills.map((s, i) => <span key={i} className="font-mono-studio flex items-center gap-1 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2.5 py-1 rounded-full text-[11px]">
                          ...
                          {matchResult.missingSkills.length > 0 && <div>
                              <p className="text-xs font-medium text-[#5C6472] dark:text-[#9AA1AC] mb-1.5">
                                Missing from your resume
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {matchResult.missingSkills.map((s, i) => <span key={i} className="font-mono-studio bg-[#E8A94C]/10 text-[#B27E2E] dark:text-[#E8A94C] px-2.5 py-1 rounded-full text-[11px]">
                                    {s}
                                  </span>)}
                              </div>
                            </div>}
            ...
            {}
            {!analysisDone && <motion.div whileHover={{
            scale: 1.01
          }} onClick={() => document.getElementById("resumeUpload").click()} className="relative rounded-2xl border-2 border-dashed border-[#E5E4E0] dark:border-[#262B34] p-7 text-center cursor-pointer hover:border-[#E8A94C]/50 hover:bg-[#E8A94C]/5 transition-all duration-200">
                ...
                <FaFileUpload className="text-3xl mx-auto text-[#B27E2E] dark:text-[#E8A94C] mb-3" />
              ...
              setResumeError("");
                ...
                {resumeFile && <motion.button whileHover={{
              scale: 1.03
            }} whileTap={{
              scale: 0.97
            }} onClick={e => {
              e.stopPropagation();
              handleUploadResume();
            }} disabled={analyzing} className="mt-4 bg-[#1C1F24] dark:bg-[#EDEEF0] text-white dark:text-[#0A0B0D] px-5 py-2 rounded-xl text-sm font-semibold transition disabled:opacity-70">
                    {analyzing ? "Analyzing..." : "Analyze Resume"}
                  </motion.button>}
              </motion.div>}

            {resumeError && <div className="bg-[#E8A94C]/8 border border-[#E8A94C]/25 rounded-xl p-3 flex items-start gap-2">
                <IoWarningOutline size={15} className="text-[#B27E2E] dark:text-[#E8A94C] mt-0.5 shrink-0" />
                <p className="text-[#8A6A2F] dark:text-[#E8B96A] text-xs leading-relaxed">
                  {resumeError}
                </p>
            ...
            <AnimatePresence>
              {analysisDone && <motion.div initial={{
              opacity: 0,
              y: 12
            }} animate={{
              opacity: 1,
              ...
              y: -8
                  ...
                  {skills.length > 0 && <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.map((s, i) => <span key={i} className="font-mono-studio bg-[#E8A94C]/10 text-[#B27E2E] dark:text-[#E8A94C] px-2.5 py-1 rounded-full text-[11px]">
                            {s}
                          </span>)}
                      </div>
                    </div>}
            ...
            {startError && <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/40 rounded-xl p-3 flex items-start gap-2">
            ...
            <motion.button onClick={handleStart} disabled={!role || !experience || loading} whileHover={{
            scale: role && experience ? 1.02 : 1
          }} whileTap={{
            scale: role && experience ? 0.98 : 1
              ...
              {loading ? "Starting interview..." : interviewType === "panel" ? "Start Panel Interview" : "Start Interview"}
            </motion.button>
          </div>
        </div>
      </motion.div>
