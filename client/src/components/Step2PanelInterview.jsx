...
function Step2PanelInterview({ interviewData, onFinish }) {
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        await speakText(
          `Hi ${userName}, welcome in. Today you'll be speaking with two of us — I'm Interviewer A, I'll focus on the technical side, and Interviewer B will cover communication and fit.`,
          "interviewerA",
        );
        await speakText(
          `We'll take turns asking questions. Just answer naturally, and take your time. Let's begin.`,
          "interviewerA",
        );
  ...
  const submitAnswer = async () => {
    ...
    try {
      ...
      const { feedback: fb, isLast, nextQuestion, testResults, testsPassedCount, testsTotalCount } =
        result.data;
      ...
      await speakText(`Thank you ${userName}. ${fb}`, activeSpeaker);
  ...
  const skipQuestion = async () => {
    ...
    try {
      ...
      await speakText(fb || "No problem, let's move on to the next question.", activeSpeaker);
