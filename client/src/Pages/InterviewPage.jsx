      ...
      <div className="interviewpage-root relative z-10 pt-6 pb-2">
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 flex items-center justify-center">
          <motion.button
            ...
            onClick={() => navigate("/")}
            title="Back to home"
            className="absolute left-4 sm:left-6 flex items-center gap-2 pl-3 pr-3.5 py-2.5 rounded-full bg-white/80 dark:bg-[#0C0E11]/80 backdrop-blur-xl border border-[#E8E6E1] dark:border-[#232830] shadow-[0_10px_28px_-12px_rgba(0,0,0,0.2)] hover:border-[#9A7B24]/50 dark:hover:border-[#E8A94C]/40 transition-colors duration-200"
          >
            <FaArrowLeft className="text-[#5B636E] cursor-pointer dark:text-[#9AA1AC]" size={12} />
            <span className="hidden sm:block cursor-pointer font-mono-studio text-[10px] tracking-[0.2em] text-[#5B636E] dark:text-[#9AA1AC]">HOME</span>
          </motion.button>

          <div className="flex items-center gap-0.5 p-1.5 rounded-full bg-white/80 dark:bg-[#0C0E11]/80 backdrop-blur-xl border border-[#E8E6E1] dark:border-[#232830] shadow-[0_14px_36px_-14px_rgba(0,0,0,0.25)]">
            {steps.map((s, i) => {
                ...
                <React.Fragment key={s.id}>
                  <motion.div
                    ...
                    className={`flex items-center gap-2.5 pl-4 pr-4 sm:pl-5 sm:pr-5 py-2.5 rounded-full transition-all duration-300 ${
                      isActive ? "bg-[#C99E41] dark:bg-[#E8A94C] shadow-[0_8px_20px_-8px_rgba(0,0,0,0.4)]" : ""
                    }`}
                  >
                    <span className={`font-mono-studio text-[10px] tracking-[0.14em] ${
                      isActive ? "text-[#14171B] dark:text-[#0A0B0D]" : isDone ? "text-[#9A7B24] dark:text-[#E8A94C]" : "text-[#8A929C] dark:text-[#565D68]"
                    }`}>
                      {String(s.id).padStart(2, "0")}
                    </span>
                    {isDone ? (
                      <BsCheckLg size={13} className="text-[#9A7B24] dark:text-[#E8A94C]" />
                    ) : (
                      <span className={isActive ? "text-[#14171B] dark:text-[#0A0B0D]" : "text-[#8A929C] dark:text-[#565D68]"}>
                        {s.icon}
                      </span>
                    )}
                    ...
                    {isActive && <span className="w-1 h-1 rotate-45 bg-[#14171B] dark:bg-[#0A0B0D]" />}
                  </motion.div>
                  {i < steps.length - 1 && (
                    <span className={`w-px h-5 transition-colors duration-300 ${isDone ? "bg-[#E8A94C]/50" : "bg-[#E8E6E1] dark:bg-[#232830]"}`} />
                  )}
                </React.Fragment>
              );
            })}
